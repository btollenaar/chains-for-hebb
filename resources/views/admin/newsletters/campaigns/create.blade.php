    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-900">Create Newsletter Campaign</h2>
            </div>

            <form method="POST" action="{{ route('admin.newsletters.campaigns.store') }}" x-data="{
                action: 'save_draft',
                showSchedule: false,
                selectedLists: [],
                recipientCount: 0,
                calculateRecipients() {
                    // This would ideally be an AJAX call, but for now we'll just show selected count
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="preview_text" id="preview_text" value="{{ old('preview_text') }}"
                            placeholder="Brief description shown in email clients"
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-admin-teal focus:ring focus:ring-admin-teal focus:ring-opacity-50">
                        @error('preview_text')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-sm text-gray-500">This appears next to the subject line in most email clients</p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="15" class="wysiwyg-editor w-full rounded-md border-gray-300 shadow-sm focus:border-admin-teal focus:ring focus:ring-admin-teal focus:ring-opacity-50">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
