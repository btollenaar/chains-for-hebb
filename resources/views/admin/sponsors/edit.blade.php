...
@section('content')
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-1">Update sponsor details for {{ $sponsor->name }}</p>
    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <form action="{{ route('admin.sponsors.update', $sponsor) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="sponsor_tier_id" id="sponsor_tier_id" required
                            ...
                            @foreach($tiers as $tier)
                                <option value="{{ $tier->id }}" {{ old('sponsor_tier_id', $sponsor->sponsor_tier_id) == $tier->id ? 'selected' : '' }}>
                                    {{ $tier->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        ...
                        @if($sponsor->logo)
                            <div class="mb-3 flex items-center gap-4">
                                <img src="{{ asset('storage/' . $sponsor->logo) }}" alt="{{ $sponsor->name }}"
                                     class="h-16 w-16 rounded object-contain border border-gray-200">
                        ...
                        @endif
                        <p class="mt-1 text-xs text-gray-500">Upload a new logo to replace the current one. Accepted formats: JPEG, PNG, SVG, WebP. Max 2MB.</p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="sponsorship_amount" id="sponsorship_amount" step="0.01" min="0" required
                                   value="{{ old('sponsorship_amount', $sponsor->sponsorship_amount) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="sponsorship_date" id="sponsorship_date" required
                                   value="{{ old('sponsorship_date', $sponsor->sponsorship_date?->format('Y-m-d')) }}"
                                   class="w-full border-gray-300 focus:border-admin-teal focus:ring-admin-teal rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <input type="date" name="sponsorship_expires_at" id="sponsorship_expires_at"
                                   value="{{ old('sponsorship_expires_at', $sponsor->sponsorship_expires_at?->format('Y-m-d')) }}"
                                   class="w-full border-gray-300 focus:border-admin-teal focus:ring-admin-teal rounded-md shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="checkbox" name="is_featured" id="is_featured" value="1"
                                   {{ old('is_featured', $sponsor->is_featured) ? 'checked' : '' }}
                                   class="h-4 w-4 text-admin-teal focus:ring-admin-teal border-gray-300 rounded">
